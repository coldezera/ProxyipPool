...
yaoyaodaili_page_header = {
    ...
    'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8',
    'Accept-Encoding': 'gzip, deflate'
    # 'Cookie': 'UM_distinctid=15e8a4194cb1c9-01ffe66ffb9564-474a0521-100200-15e8a4194cc338; safedog-flow-item=; CNZZDATA1256284042=339433765-1505552968-%7C1506311067'
}
...
class CheckIP:
    ...
    # def parse_ip(url):# guobanjia (0/210的可用率！）(Xpath语法运用，所以此函数值得保留)
    #     res = requests.get(url, headers=page_header)
    #     tree = etree.HTML(res.text)
    #     # print(res.text)
    #     for ip_detail in tree.xpath('//table[@class="table"]/tbody/tr'):
    #         ip_addr = ''
    #         ip_list = ip_detail.xpath(""".//td[1]/*[not(contains(@style, 'display: none'))
    #                                        and not(contains(@style, 'display:none'))
    #                                        and not(contains(@class, 'port'))
    #                                        ]/text()""")
    #         port = ip_detail.xpath('.//td[1]/span[contains(@class, "port")]/text()')[0]
    #         ip_addr = ''.join(ip_list)
    #         prot = ip_detail.xpath('.//td[3]/a/text()')[0]
    #         unverify_ip_list.append({
    #             'ip:port': '{}:{}'.format(ip_addr, port),
    #             'protocol': prot
    #         })
    #     pass
